...
-- questao 1 Liste todos os pedidos com o nome do cliente correspondente
select clientes.nome, pedidos.id as pedido from pedidos
inner join clientes on pedidos.cliente_id =clientes.id;

-- questao 2 Quantos pedidos foram feitos no total?
select count(pedido_id) from itens_pedido where pedido_id;

-- questao 3 Liste os pedidos feitos após '2016-12-15' com o nome do cliente
select pedidos.id as pedido, pedidos.data, cliente.nome from pedidos
inner join clientes on pedidos.cliente_id = clientes.id
...
-- questao 6 Qual o valor total de todos os pedidos feitos até agora
select sum(valor)  as 'valor' from pedidos ;

-- questao 7 
-- questao 8 Liste todos os clientes e seus pedidos feitos no mês de dezembro de 2016 (inclua clientes sem pedidos).
...
-- questao 09 . Qual o valor médio dos pedidos realizados
...
left join clientes on clientes.id = pedidos.id
group by clientes.id, clientes.nome;

-- questao 11  Qual o valor do pedido mais caro registrado?

select max(valor) as'maior valor' from pedidos;
-- questao 12  Qual o valor do pedido mais barato registrado?
select min(valor) as'menor valor' from pedidos;

-- questao 13 Quantos pedidos cada cliente fez (mesmo que não tenham feito nenhum)?
 select pedidos.id as cliente, nome, pedidos.id as pedidos from clientes 
 left join pedidos on pedidos.cliente_id = cliente_id;
 
 -- questao 14  Qual o pedido mais caro, exibir o nome do cliente e o valor do pedido.
 select pedidos.id as clientes, nome, pedidos.id as pedidos, max(valor) from clientes 
 left join pedidos on pedidos.cliente_id = cliente_id;
...
select pedidos.id as clientes, nome, pedidos.id as pedidos, max(valor) ,min(valor) from clientes 
 left join pedidos on pedidos.cliente_id = cliente_id
 where clientes.nome = 'Laura Madureira';

-- questao 18  Quantas pizzas cada cliente comprou no total
